...
export function CeldaInfoModal({
  ...
  useEffect(() => {
    setConfirmandoEstacionar(false);
  }, [open, celdaActiva?.id]);
  ...
  const permiso = agenda
    ? puedeEstacionarEn(agenda, null)
    : { puede: true as const };
  const aviso = agenda
    ? avisoDeCelda(agenda, celdaActiva?.estado === "no_disponible")
    : null;
  ...
  const enReserva =
    ...
    (celdaActiva?.estado === "disponible" && !!reservaActiva);
  const colorAviso =
    aviso?.tono === "urgente"
    ...
    <Modal open={open} onClose={onClose} maxWidth={480}>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          >
            {celdaActiva?.estado === "no_disponible" && ocupanteActivo
              ? ocupanteActivo.vehiculo.placa
              : enReserva
                ? vehiculoReservado?.placa || "Reservada"
                : "Celda Libre"}
          </h2>
          {celdaActiva?.estado === "no_disponible" && ocupanteActivo && (
            <p
            ...
            >
              {ocupanteActivo.conductor?.nombre || "—"}
            </p>
          )}
          <div
            style={{
          ...
          >
            {celdaActiva && <TipoBadge tipo={celdaActiva.tipo} />}
          </div>
        </div>
      </div>
      <div
      ...
      >
        {canManageCeldas && celdaActiva && onSetEstadoManual && (
          ...
          >
            ...
            {ocupanteActivo ? (
              ...
              >
                La celda está ocupada. Debe registrarse la salida del vehículo
              ...
              <>
                ...
                <p style={{ fontSize: 9, color: C.textLight, marginTop: 6 }}>
                  Cambia el estado sin pasar por el flujo normal
                  (estacionar/reservar/liberar). Úsalo solo para corregir una
                  celda atascada o ponerla en mantenimiento.
                </p>
                ...
                {agenda?.proxima && (
                  ...
                  >
                    Esta celda tiene una reserva a las{" "}
                    {comoHora(inicioDe(agenda.proxima))}: cambiar el estado a
                    mano no la cancela.
                  </p>
                )}
              </>
        ...
        {aviso && (
          <div
            style={{
              padding: "12px 14px",
              borderRadius: 11,
              background: colorAviso.fondo,
              border: `1px solid ${colorAviso.borde}`,
              fontSize: 12,
              color: colorAviso.texto,
              fontWeight: 700,
            }}
          >
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 9,
                      background: "#fff",
                      border: `1px solid ${esVigente ? C.primaryLight : C.border}`,
                    }}
                  >
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      >
                        {detalle?.placa || "—"}
                        {detalle?.conductor ? ` · ${detalle.conductor}` : ""}
                      </div>
                    </div>
                    <span
                      style={{
        ...
        {celdaActiva?.estado === "mantenimiento" && (
          ...
          >
            🔧 Esta celda está en mantenimiento y no acepta vehículos ni
            reservas.
          </div>
        )}
        {enReserva ? (
          <>
            ...
            >
              {reservaActiva
                ? `Celda reservada hasta las ${reservaActiva.horaFin}.`
                : "Celda reservada."}
            </div>
            {reservaActiva &&
              [
                {
                  icon: Car,
                  label: "Vehículo",
                  value: vehiculoReservado
                    ? `${vehiculoReservado.placa} — ${vehiculoReservado.marca} ${vehiculoReservado.modelo}`
                    : "—",
                },
                ...
                // está guardando la celda.
                {
                  icon: UserCircle2,
                  label: "Reservada por",
                  value: conductorReserva || "—",
                },
                {
                  icon: Calendar,
                  label: "Fecha",
                  value: reservaActiva.fechaReserva,
                },
                {
                  icon: ClockIcon,
                  label: "Horario",
                  value: `${reservaActiva.horaInicio} – ${reservaActiva.horaFin}`,
                },
              ].map((r, i) => (
                <div
                  key={i}
            ...
            {parqueaderoInactivo && (
              ...
              >
                Este parqueadero está inactivo: no se pueden registrar nuevos
                ingresos.
              </div>
            )}
            {canRegistrarIngreso && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                {reservaActiva && (
                  <button
                    onClick={() => onCancelarReserva(reservaActiva)}
                    style={{
                      flex: 1,
                      padding: "10px",
                      borderRadius: 11,
          ...
          <>
            {estaFueraDeHorarioOperacion() && (
              ...
              >
                ⏰ Este vehículo sigue estacionado fuera del horario permitido
                (hasta las {HORA_OPERACION_FIN}).
                {canReportarIncidentes
                  ? " Considera generar un incidente."
                  : ""}
              </div>
            )}
            {[
                ...
                ? [
                    {
                      icon: UserCircle2,
                      label: "Conductor",
                      value: ocupanteActivo.conductor.nombre,
                      onClick: () =>
                        onNavigateConductor(ocupanteActivo.conductor!.nombre),
                    },
                  ]
                : []),
              {
                icon: MapPin,
                label: "Parqueadero",
                value: parqueaderoActivo?.nombre,
              },
              {
                icon: Clock,
                label: "Ingreso",
                value: `${formatearFechaHora(ocupanteActivo.fechaEntrada).fecha} ${formatearFechaHora(ocupanteActivo.fechaEntrada).hora}`,
              },
              {
                icon: Clock,
                label: "Estadía",
                value: formatearDuracion(ocupanteActivo.fechaEntrada),
              },
            ].map((r, i) => (
              <div
                key={i}
                ...
                onMouseEnter={
                  r.onClick
                    ? (e) => (e.currentTarget.style.background = "#F1F5F9")
                    : undefined
                }
                onMouseLeave={
                  r.onClick
                    ? (e) => (e.currentTarget.style.background = "#F8FAFC")
                    : undefined
                }
              >
                <r.icon size={14} color={C.textLight} />
                <div>
                  ...
                  <div
                    style={{
                      fontSize: 13,
                      fontWeight: 600,
                      color: r.onClick ? C.primary : C.text,
                    }}
                  >
                    {r.value}
                  </div>
            ...
            {(canRegistrarIngreso || canReportarIncidentes) && (
              ...
              >
                ...
                {canReportarIncidentes && (
                  <button
                    onClick={
                      incidenteAbiertoExiste ? undefined : onReportarIncidente
                    }
                    disabled={incidenteAbiertoExiste}
                    title={
                      incidenteAbiertoExiste
                        ? "Ya existe un incidente abierto para esta celda o vehículo."
                        : "Reportar Incidente"
                    }
                    style={{
                      display: "flex",
          ...
          parqueaderoInactivo ? (
            ...
            >
              ⚠️ Este parqueadero está inactivo y no acepta nuevos registros.
              Actívalo en el módulo Parqueaderos.
            </div>
          ) : (
            <>
              {permiso.puede ? (
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                {canRegistrarIngreso &&
                  ...
                  vehiculoReservado && (
                    ...
                    >
                      Estacionar {vehiculoReservado.placa} — tiene la reserva de
                      las {comoHora(inicioDe(agenda.proxima))}
                    </button>
                  )}
                {canRegistrarIngreso &&
                  !confirmandoEstacionar &&
                  !(agenda?.proxima && vehiculoReservado) && (
                    <button
                      onClick={
                        permiso.confirmacion
                          ? () => setConfirmandoEstacionar(true)
                          : onEstacionarVehiculo
                      }
                      ...
                      style={{
                        ...
                        borderRadius: 11,
                        border: "none",
                        background: permiso.puede ? C.primary : C.border,
                        color: "#fff",
                        fontSize: 12,
                        fontWeight: 800,
                        cursor: permiso.puede ? "pointer" : "not-allowed",
                        fontFamily: "inherit",
                        boxShadow: permiso.puede
                          ? "0 4px 14px rgba(57,169,0,.25)"
                          : "none",
                      }}
                    >
                      Estacionar Vehículo
