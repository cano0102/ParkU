...
describe("features/conductores", () => {
  ...
  it("alterna entre vista de cuadrícula y de lista", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("Centro de formación")).toBeInTheDocument(),
    );
    expect(screen.getByText("Vehículo(s)")).toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: "Cuadrícula" }));
    await waitFor(() =>
      expect(screen.queryByText("Centro de formación")).not.toBeInTheDocument(),
    );
  });
  ...
  it("al vincular una cuenta muestra sus datos y deja el correo no editable", async () => {
    ...
    const correo = within(dialog).getByPlaceholderText(
      "correo@sena.edu.co",
    ) as HTMLInputElement;
    const documento = within(dialog).getByPlaceholderText(
      "ej. 1001234567",
    ) as HTMLInputElement;
    ...
    await user.type(
      within(dialog).getByPlaceholderText("Buscar por nombre o correo..."),
      "maria",
    );
    ...
    expect(
      within(dialog).getByText("Correo (de la cuenta vinculada)"),
    ).toBeInTheDocument();
    expect(
      within(dialog).getByText("Tipo de documento (de la cuenta)"),
    ).toBeInTheDocument();

    // Al quitar la vinculación vuelven a editarse, sin perder el resto del formulario.
    await user.click(
      within(dialog).getByLabelText("Quitar la cuenta vinculada"),
    );
  ...
  it("elimina un conductor tras confirmarlo, avisando que su cuenta no se borra", async () => {
    ...
    expect(
      (await screen.findAllByText("Eliminar conductor")).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByText(/Su cuenta de acceso y sus vehículos NO se borran/),
    ).toBeInTheDocument();
  ...
  it("no elimina al conductor si se cancela la confirmación", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryAllByText("Eliminar conductor")).toHaveLength(0),
    );
    expect(screen.getAllByText("Carlos López M.").length).toBeGreaterThan(0);
  }, 20000);
  ...
  it("abre el modal de creación al hacer clic en Nuevo Conductor", async () => {
    ...
    expect(
      within(dialog).getByLabelText("Tipo de usuario"),
    ).toBeInTheDocument();
    // Y se puede crear la cuenta desde aquí mismo.
    expect(
      within(dialog).getByLabelText("No tengo usuario"),
    ).toBeInTheDocument();
    ...
    expect(
      within(dialog).getByPlaceholderText("Ej: ABC123"),
    ).toBeInTheDocument();
  });

  it("incluye la opción Otro dentro del catálogo de tipos de usuario del conductor", async () => {
    const user = userEvent.setup();
    renderConductores();
    await waitFor(() =>
      expect(screen.getAllByText("Carlos López M.").length).toBeGreaterThan(0),
    ...
    expect(
      within(dialog).getByRole("option", { name: "Otro" }),
    ).toBeInTheDocument();
  });
  ...
  it("vincular un vehículo existente como copropietario lo muestra en la tarjeta del nuevo conductor", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Agregar vehículo a Pedro Ruiz G." }),
    );
    const dialog = await screen.findByRole("dialog");

    await user.click(
      within(dialog).getByRole("button", { name: "Vincular existente" }),
    );
    await user.click(await within(dialog).findByText(/ABC123/));
    await user.click(
      within(dialog).getByRole("button", { name: "Vincular Copropietario" }),
    );
    ...
    await waitFor(() => {
      const tarjetaPedro = screen
        .getByText("Pedro Ruiz G.")
        .closest(".conductor-card") as HTMLElement;
  ...
  it("abre el detalle de un conductor al hacer clic en Ver detalles", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Ver detalles de Carlos López M." }),
    );

    const dialog = await screen.findByRole("dialog");
    expect(
      within(dialog).getByRole("heading", { name: "Carlos López M." }),
    ).toBeInTheDocument();
